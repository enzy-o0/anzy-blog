...
**사이트는 완전한 정적 export다.** `output: "export"`이므로 서버 런타임에 의존하는 것을 추가하지 않는다 — 서버 액션, 라우트 핸들러, `next/image` 최적화, 미들웨어, ISR. 필요해지면 호스팅부터 다시 논의한다.

**절대경로 자산은 `withBasePath()`를 통과시킨다.** 하위 경로 배포에서 마크다운의 raw `<img>` / `<a>`는 `basePath`가 자동으로 붙지 않는다. `lib/base-path.ts`가 단일 출처이고, 값이 갈리면 페이지는 뜨는데 이미지만 404가 나는 방식으로 깨진다.
