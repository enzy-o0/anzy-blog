...
description: 2년 전 과제 코드를 다시 읽고, any가 어디에 놓였는지 확인한 기록
date: "2026-08-12"
categories:
  - NextJS
tags:
...
2년 전에 낸 과제 코드를 포트폴리오로 쓰려고 다시 열었다. 마크다운을 읽어 정적 페이지로 굽는 작은 블로그다. 동작은 했다. 배포도 되어 있었다.
...
같은 메시지로 두 번. 그때의 나는 "일단 돌아가게 하고 나중에 고치자"고 생각했을 것이다. 나중은 오지 않았다.
...
`any`는 타입 검사를 **건너뛴** 게 아니다. 이미 있던 타입을 **무력화**했다. 그게 더 나쁘다. 타입이 없으면 없다는 걸 알기라도 하는데, 있는데 거짓말을 하고 있으면 그걸 믿게 된다.
...
같은 눈으로 다시 읽으니 다른 것들도 보였다. 대부분 "동작하니까 넘어간" 것들이다.

**하이라이팅을 문자열 치환으로 흉내내고 있었다.**
...
마크다운을 HTML로 바꾼 뒤 결과 문자열에 클래스를 끼얹는다. 코드블록의 언어가 뭔지 알 방법이 없고, 본문에 `<pre>`가 등장하면 그것까지 같이 오염된다. 실제 색칠은 클라이언트에서 했기 때문에 하이라이팅 하나 때문에 highlight.js 전체가 브라우저로 내려갔다.

**글 하나를 추가하려면 세 곳을 고쳐야 했다.** 마크다운 파일, `getStaticPaths`의 배열, 그리고 홈 화면의 카드 JSX. 목록의 진실이 세 군데에 흩어져 있었고 서로 동기화되지 않았다.

**`fs`라는 패키지를 설치해뒀다.** `package.json`에 `"fs": "0.0.1-security"`가 있었다. 이건 실제 패키지가 아니다. npm이 이름 스쿼팅을 막으려고 올려둔 빈 자리표시자다. Node 내장 `fs`를 쓰려고 설치한 것 같은데, 오히려 내장 모듈을 가린다. `path`도 마찬가지였다.

미사용 의존성이 6개 더 있었고, `yarn.lock`과 `package-lock.json`이 함께 커밋되어 있었다.
