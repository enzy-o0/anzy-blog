...
<!-- TODO(직접): 본인 코드인데 "생각했을 것이다"라고 추측하면 이상하다.
     그때 왜 any로 뒀는지 한두 문장. 마감이었나, 몰랐나, 알면서 미뤘나.
     그리고 다시 열었을 때 무슨 생각이 들었는지도 여기에. -->
...
<!-- TODO(직접): 그런데 이게 4년간 한 번도 안 터졌다.
     터지지 않은 버그를 버그라고 불러도 되나? 왜 지금 와서 고쳤나?
     답을 내릴 필요는 없고, 이 질문을 인정하는 문단 하나가 들어가면 글이 정직해진다. -->
...
<!-- TODO(직접): 다음 글을 정말 쓸 거면 아래 줄을 남기고, 아니면 지운다.
     안 쓰면 빈 약속으로 남는다. -->
